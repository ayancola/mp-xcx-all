...
Page({
    ...
    viewimgAlbum: function (e) {
        // console.log(e);
        let currentImg = e.currentTarget.dataset.img;
        let img_arr = [];
        for (var i = 0; i < this.data.album_img.length; i++) {
            img_arr.push(this.data.album_img[i].images);
        }
        wx.previewImage({
    ...
    onLoad(e) {
      ...
      wx.setNavigationBarTitle({title:this.data.navigationTitle });
        ...
        aboutModel.getPhoto(_this.data.album_rid,function (msg) {
          _this.setData({album_img:msg.data})
        });
